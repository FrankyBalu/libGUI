...
{
	auto file_logger = spdlog::rotating_logger_mt("test_Button_Widget", "logs/test_HBox.log", 1048576 * 5, 5);
    ...
    LIBGUI::cHBox *hbox;
    hbox = new LIBGUI::cHBox( win );
    LIBGUI::newButton *button1;
    LIBGUI::newButton *button2;
    button1 = new LIBGUI::newButton (hbox, "Test_Button", "button1" );
    button2 = new LIBGUI::newButton (hbox, "Test_Button", "button2" );
    //button->load();
    while ( win->isOpen() )
    {
    	win->update (ev);
